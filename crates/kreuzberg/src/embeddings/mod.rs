...
fn resolve_model_info(
    ...
    match model_type {
        ...
        crate::core::config::EmbeddingModelType::Plugin { .. } => Err(crate::KreuzbergError::embedding(
            "Plugin embeddings are routed to the registered backend and do not use ONNX model resolution",
        )),
    }
}
...
pub fn embed_texts<T: AsRef<str>>(
    ...
    match &config.model {
        ...
        crate::core::config::EmbeddingModelType::Plugin { name } => {
            ...
            let timeout = config.max_embed_duration_secs.map(std::time::Duration::from_secs);
...
pub async fn embed_texts_async<T: AsRef<str> + Send + 'static>(
    ...
    // Plugin-dispatched embeddings await the host-language backend directly;
    // no spawn_blocking round-trip is needed since the trait is already async.
    if let crate::core::config::EmbeddingModelType::Plugin { name } = &config.model {
        let registry = crate::plugins::get_embedding_backend_registry();
        let (backend, expected_dim) = {
            let guard = registry.read();
            guard.get_with_dimensions(name)?
