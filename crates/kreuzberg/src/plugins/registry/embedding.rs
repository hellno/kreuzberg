...
impl EmbeddingBackendRegistry {
    ...
    /// Create a new empty embedding backend registry.
    ///
    /// Equivalent to [`Self::new`] for this registry type. Kept for API
    /// symmetry with [`super::OcrBackendRegistry`], where `new` registers
    /// built-in backends and `new_empty` does not — the embedding registry
    /// has no built-ins to register (all backends come from the host
    /// language), so the two constructors collapse to the same behaviour.
